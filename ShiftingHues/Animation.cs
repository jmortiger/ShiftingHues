...
namespace ShiftingHues.Graphics
{
	...
	public class Animation
	{
		#region Properties
		/// <summary>
		/// The sprites to draw. Each sprite takes 1 frame.
		/// </summary>
		public SpriteInstance[] AnimationFrames { get; private set; }
		public DrawEffects2D[] SuggestedDrawEffects { get; private set; }
		public int NumFrames { get => AnimationFrames.Length; }
		...
		public Animation(Sprite[] animationFrames, DrawEffects2D? drawEffects)
			: this(false, drawEffects ?? new DrawEffects2D())
		{
			this.AnimationFrames = new SpriteInstance[animationFrames.Length];
			for (int i = 0; i < animationFrames.Length; i++)
				this.AnimationFrames[i] = new SpriteInstance(animationFrames[i], new DrawEffects2D(animationFrames[i].SourceRect));
		}

		public Animation(SpriteInstance[] animationFrames)
